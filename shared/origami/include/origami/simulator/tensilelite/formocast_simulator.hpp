...
class Formocast {
 public:
   ...
   */
  MemoryAccessCosts calculateMemoryAccessCosts(double MT0,
                                               double MT1,
   ...
   */
  double getLoopOverall(const MemoryAccessCosts& mem,
                        double math,
                        uint32_t loopCnt,
                        double pgr) const;
  ...
  /**
   * @brief Extract intermediate performance features as a flat vector.
   *
   * Runs the full prediction pipeline and returns physics-informed features
   * suitable for ML-based ranking models. Features include cache hit rates,
   * memory costs per cache level, compute/loop/store costs, and overheads.
   *
