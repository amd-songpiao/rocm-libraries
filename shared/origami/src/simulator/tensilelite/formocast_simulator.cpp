...
void Formocast::calculateStorePerformance(double M,
                                          double N,
                                          double NumBatches,
                                          double MT0,
                                          double MT1,
...
Formocast::MemoryAccessCosts Formocast::calculateMemoryAccessCosts(double MT0,
                                                                   double MT1,
                                                                   const HardwareConstants& hw,
                                                                   const CacheHitRates& hr,
                                                                   double L2BandWidthPerCU,
                                                                   double L3BandWidthPerCU,
                                                                   double HBMBandWidthPerCU,
                                                                   bool isSwizzleA,
                                                                   bool isSwizzleB,
                                                                   double A_L1_req,
                                                                   double B_L1_req,
...
Formocast::PredictedPerformance Formocast::predictedPerformance(void) const {
  ...
  PGR                       = (std::floor(K_AfterGSU / depthU > 1)) ? sizeMapping.PrefetchGlobalRead
                                                                    : int(K_AfterGSU / depthU);
  int PLR                   = (std::floor(K_AfterGSU / sizeMapping.LocalSplitU / depthU) < 1)
                                  ? 0
                                  : 1;  // sizeMapping.PrefetchLocalRead;
  ...
  double A_L1_req = simulator::getLoadRequest(
      std::min(MT0, M),
      depthU,
      hw_consts.L1CacheLineSize,
      GRVWA,
  ...
  double B_L1_req = simulator::getLoadRequest(
      std::min(MT1, N),
      depthU,
      hw_consts.L1CacheLineSize,
      GRVWB,
  ...
  MemoryAccessCosts mem_costs = calculateMemoryAccessCosts(std::min(MT0, M),
                                                           std::min(MT1, N),
                                                           hw_consts,
                                                           cache_hits,
                                                           L2BandWidthPerCU,
  ...
  double prefetch   = getPrefetchPerformance(
  ...
  perf = resolveOccupancy(
      hw_consts, perf, prefetch, loop_overall + tail_overall, store, num_tiles, CUOccupancy);
